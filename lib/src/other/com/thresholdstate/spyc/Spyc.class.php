...
// LOADING FUNCTIONS
  ...
  private function addArray ($array, $indent) {
    ...
    if ($key)
      /*---------------------------------------------------------------------
       * FURNACE-SPECIFIC MODIFICATION
       * --------------------------------------------------------------------
       * The below is a non-standard addition which allows for *much* simpler
       * application model syntax, at the expense of generality. Since
       * Furnace uses YML exclusively for parsing models, however, the
       * impact of this change is net positive. 
       * 
       * The change affects the way arrays are processed. Specifically,
       * it allows colliding array keys to be appended rather than 
       * overwritten. 
       * 
       * Example yaml (before this addition):
       * 
       * BlogPost:
       *   _belongsTo: user
       *   _belongsTo: blog
       ...
       * would generate:
       * ...
       * ["BlogPost"]=> array ( "_belongsTo" => "blog", "body" => "text")
       * 
       * which, according to standard yaml, swallows the first _belongsTo
       * since it encounters a subsequent, colliding array key. The below
       * first checks to see whether the key exists, and, if so, wraps the
       * existing value in an array before appending the current value to it.
       * In this way, colliding array keys are preserved.
       * 
       * With the addition, the same example yaml above would generate:
       * 
       * ["BlogPost"]=> array ( 
       * 	"_belongsTo" => array("user","blog"), "body" => "text")
       * 
       * which preserves both _belongsTo elements as desired.
       * --------------------------------------------------------------------
       */
      if (isset($_arr[$key])) {
      	// The key already exists in the array. What to do?
        if (is_array($_arr[$key])) {
          // Since the existing value is already an array, simply
          // append the current value to it
          $_arr[$key][] = $value;
        } else {
      	  // Create an array out of the existing scalar value and 
      	  // append the current value to it
          $_arr[$key] = array($_arr[$key],$value);
      	}
      } else {
      	$_arr[$key] = $value;
      }
