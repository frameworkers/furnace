...
class TadpoleEngine {
	...
	public function loadPageData($data,$bAppend = false) {
		$this->page_data = ($bAppend) ? array_merge($this->page_data,$data) : $data;
	}
		...
		while (1) {
			...
			if (';' == $contents[$signalOffset]) {
				...
				while (';' == $contents[$signalOffset]) {
					...
					// Skip any escaped semi-colons (\;)
					$commandTest = $commandStart;
					
					do {
					$commandTest    = $signalOffset + 1;
					$signalOffset   = (($signalOffset = strcspn($contents, ";".self::TAG_END_CHAR,$commandTest)) != strlen($contents)-($commandTest))	
						? $signalOffset + $commandTest 
						: false;
					} while ($signalOffset && $contents[$signalOffset-1].$contents[$signalOffset]  == '\\;');
					...
					while (false !== ($nestedStart = strpos(substr($contents,$temp,($secondOffset - $temp)),self::TAG_START_CHAR))) {	// BUGGER RIGHT HERE, ($signalOffset - $temp is not right)
						...
						// Skip any escaped semi-colons (\;)
						$secondTest = $bracketOffset + 1;
						do { 
							$secondTest     = $secondOffset + 1;
							$secondOffset   = (($secondOffset   = strcspn($contents, ";".self::TAG_END_CHAR,$secondTest + 1)) != strlen($contents)-($secondTest + 1))
								? $secondOffset + $secondTest 
								: false;
						} while ($secondOffset && $contents[$secondOffset].$contents[$secondOffset+1] == '\\;'); // This one is +1 b/c of funny business on next line
						$secondOffset = $secondOffset + 1;   // swallow the TAG_END_CHAR
					...
					// Extract the current command from the tag, also, replace any escaped semi-colons (\;)
					$commandEnd = $signalOffset;
					$command    = str_replace('\;',';',substr($contents,$commandStart, ($commandEnd - $commandStart)));
	...
	public function regularBlockHandler($tag,&$context,$iter_data) {
		...
		$cnt  = 0;
		foreach ($block_data as $rowKey => $rowData) {
		    //for ($rowIdx = 0,$rows = count($block_data);$rowIdx < $rows;$rowIdx++) {
			$completeBlockContents .= $this->compile($compiledBlockContents,array($rowKey => $rowData));
	...
	public function conditionalBlockHandler($tag,&$context,$iter_data) {
		...
		if ($conditionHeld) {
			...
			if (isset($context['commands']['innerelse']) && $context['commands']['block'] != "self" && !$context['isPseudo']) {
				...
				$before   = substr($context['contents'], 0, $context['blockStart'] +  ((strpos($context['contents'], ">",$context['blockStart'])+1) - $context['blockStart']));
				...
				case 'type':
					switch ($commands['type']) {
						...
						case 'date':
							...
							if (!isset($commands['unix'])) {	// Convert to unix timestamp
	...
	protected function get_recursively($needle,&$commands,&$rejected,$iter_data=array()) {
		...
		if ("@" == $needle) { return (is_array($iter_data) ? reset($iter_data) : $iter_data);}
		...
		// Otherwise, decompose and process the needle here
			...
			if ($relative) {
				// Use ITERATION DATA
				$flashlight = is_array($iter_data) ? reset($iter_data) : $iter_data;
			...
			foreach ($parts as $segment) {
				...
				// Handle the case in which the 'key' name is requested (applies to arrays only)
				if ('@' == $segment && is_array($iter_data) && !empty($iter_data)) {
					$keys = array_keys($iter_data);
					return $keys[0];
				}
				
				if ($bIsObject) {
					$bExists = (isset($flashlight->$segment) || is_callable(array($flashlight,$privateMethod))
						|| '#' == $segment);
				} else {
	...
	public function reset() {
		...
		$this->conditionalCache = $this->tagCache =
			$this->relativeConditionalCache = $this->relativeTagCache = array();
	}
