...
class Dataset implements \Iterator, \Countable, \ArrayAccess, ISqlSelectOptions, ISqlConditions {
	...
	/**
	 * Delete the members of this dataset
	 * Right now, this only works if ::realize() has not yet been called. If realize
	 * has not been called, it is possible to operate on all 'members' of the dataset
	 * at once. If realize HAS been called, then each of the realized members must be
	 * individually deleted using that object's ->delete method. This {sh|c}ould be 
	 * implemented here later.
	 * Enter description here ...
	 */
	public function delete() {
		return $this->source->executeSqlBuilder($this->sqlBuilder,'delete',$this->source);
	}
	...
	function assign($column, $value) { 
		$copy = clone $this; 
		$copy->sqlBuilder->assign($column, $value); 
		return $copy; 
	}
	 ...
	 * @return PdoDataSet
	 */
	function useAssignmentsAsConditions($bool) { 
		$copy = clone $this; 
		$copy->sqlBuilder->useAssignmentsAsConditions($bool); 
		return $copy; 
	}
	...
	function leftOuterJoin($table, $tablePrimaryKey, $fromTableForeignKey) { 
		$copy = clone $this; 
		$copy->sqlBuilder->leftOuterJoin($table, $tablePrimaryKey, $fromTableForeignKey); 
		return $copy; 
	}
	...
	function innerJoin($table, $tablePrimaryKey, $fromTableForeignKey) { 
		$copy = clone $this; 
		$copy->sqlBuilder->innerJoin($table, $tablePrimaryKey, $fromTableForeignKey); 
		return $copy; 
	}
	 ...
	 * @return PdoDataSet
	 */	
	function selectAs($select, $as) { 
		$copy = clone $this; 
		$copy->sqlBuilder->selectAs($select, $as); 
		return $copy; 
	}
	...
	function notEqual($lhs, $rhs) { 
		$copy = clone $this; 
		$copy->sqlBuilder->notEqual($lhs,$rhs); 
		return $copy; 
	}
	 ...
	 * @return PdoDataSet
	 */	
	function between ($column, $lhs, $rhs) { 
		$copy = clone $this; 
		$copy->sqlBuilder->between($column, $lhs, $rhs); 
		return $copy; 
	}
	...
	function greaterThan($lhs, $rhs) { 
		$copy = clone $this;
		$copy->sqlBuilder->greaterThan($lhs,$rhs); 
		return $copy; 
	}
	...
	function greaterThanOrEqualTo($lhs, $rhs) { 
		$copy = clone $this;
		 $copy->sqlBuilder->greaterThanOrEqualTo($lhs,$rhs); 
		 return $copy; 
	}
	...
	function lessThan($lhs, $rhs) { 
		$copy = clone $this; 
		$copy->sqlBuilder->lessThan($lhs,$rhs); 
		return $copy; 
	}
