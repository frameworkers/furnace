...
use org\frameworkers\furnace\persistance\orm\pdo\sql\ISqlConditions;
use org\frameworkers\furnace\persistance\orm\pdo\sql\ISqlSelectOptions;
use Exception;
...
class SqlBuilder implements ISqlConditions, ISqlSelectOptions {
	...
	protected function insertSanityCheck() {
		if(	!empty($this->conditions) )
			throw new Exception('Insert does not use conditionals.', get_defined_vars());
		if(	!empty($this->joins) )
			throw new Exception('Insert does not use joins.', get_defined_vars());
		if(	!empty($this->orderBy) ) 
			throw new Exception('Insert does not use order by.', get_defined_vars());
		if(	isset($this->limit) )
			throw new Exception('Insert does not use limit.', get_defined_vars());
		if(	isset($this->offset) )
			throw new Exception('Insert does not use offset.', get_defined_vars());
		if(	isset($this->distinct) )
			throw new Exception('Insert does not use distinct.', get_defined_vars());
	}
	...
	public function assign($column, $value) { 
		if(strpos($column, '.') === false) {
			if(isset($this->table)) {
				$this->assignments[] = new Criterion($this->tableAsPrefix() 
					. '.' 
					. $column, $value, Criterion::ASSIGNMENT); 
			} else {
				throw new Exception(
					'Cannot assign without specifying table.', get_defined_vars());
			}
		} else {
	...
	protected function deleteSanityCheck() {
		if(	!empty($this->joins) )
			throw new Exception('Delete does not use joins.', get_defined_vars());
		if(	!empty($this->orderBy) ) 
			throw new Exception('Delete does not use order by.', get_defined_vars());
		if(	isset($this->limit) )
			throw new Exception('Delete does not use limit.', get_defined_vars());
		if(	isset($this->offset) )
			throw new Exception('Delete does not use offset.', get_defined_vars());
		if(	isset($this->distinct) )
			throw new Exception('Delete does not use distinct.', get_defined_vars());
		if( !empty($this->assignments) && !$this->useAssignmentsAsConditions)
			throw new Exception('Delete does not use assignments. To use assignments "
				."as conditions add ->useAssignmentsAsConditions() to your method call chain.'
				, get_defined_vars());
	}
	...
	public function update() {
		...
		foreach($this->assignments as $assignment) {
			...
			$sql .= (self::escapeWithTicks(str_replace($table_prefix, '', $assignment->column)) 
				 . ' = ' . $assignment->getQueryParameter());
		}
	...
	protected function updateSanityCheck() {
		if(	!empty($this->joins) )
			throw new Exception('Update does not use joins.', get_defined_vars());
		if(	!empty($this->orderBy) ) 
			throw new Exception('Update (in Furnace) does not use order by.', get_defined_vars());
		if(	isset($this->limit) )
			throw new Exception('Update (in Furnace) does not use limit.', get_defined_vars());
		if(	isset($this->offset) )
			throw new Exception('Update (in Furnace) does not use offset.', get_defined_vars());
		if(	isset($this->distinct) )
			throw new Exception('Update does not use distinct.', get_defined_vars());
	}
	...
	public function useAssignmentsAsConditions($bool) { 
		$this->useAssignmentsAsConditions = $bool; return $this; 
	}
	...
	public function equal($column, $value) { 
		return $this->addCondition($column, $value, Criterion::EQUAL_TO); 
	}
	...
	public function notEqual($column, $value) { 
		return $this->addCondition($column, $value, Criterion::NOT_EQUAL_TO); 
	}
	 ...
	 * @return SqlBuilder
	 */
	public function between ($column, $small, $big) { 
		$this->greaterThan($column, $small); 
		return $this->lessThan($column, $big); 
	}
	...
	public function greaterThan($column, $value) { 
		return $this->addCondition($column, $value, Criterion::GREATER_THAN); 
	}
	...
	public function greaterThanOrEqualTo($column, $value) { 
		return $this->addCondition($column, $value, Criterion::GREATER_THAN_EQUAL_TO); 
	}
	...
	public function lessThan($column, $value) { 
		return $this->addCondition($column, $value, Criterion::LESS_THAN); 
	}
	...
	public function lessThanOrEqualTo($column, $value) { 
		return $this->addCondition($column, $value, Criterion::LESS_THAN_EQUAL_TO); 
	}
	...
	public function like($column, $value) { 
		return $this->addCondition($column, $value, Criterion::LIKE); 
	}
	...
	public function notLike($column, $value) { 
		return $this->addCondition($column, $value, Criterion::NOT_LIKE); 
	}
	...
	public function isNull($column)  { 
		return $this->addCondition($column, null, Criterion::IS_NULL); 
	}
	...
	public function isNotNull($column) { 
		return $this->addCondition($column, null, Criterion::IS_NOT_NULL); 
	}
	...
	protected function addCondition($column, $value, $operator) {
		if(strpos($column, '.') === false && 
			strpos($column, '(') === false && 
			!in_array($column, array_keys($this->selectAs))) {
			if(isset($this->table)) {
				$column = $this->tableAsPrefix() . '.' . $column;
			} else {
				throw new Exception('Cannot use "' . $operator 
					. '" operator without specifying table for column "' 
	...
	protected function selectSanityCheck() {
		if( (!empty($this->where) || !empty($this->orderBy)) && !isset($this->table))
			throw new Exception('Must have from if using where.');
		
		if( isset($this->offset) && !isset($this->limit))
			throw new Exception('Must define limit if using offset.');
	...
	/**
	 * When used in conjunction with limit($size), offset specifies which 
	 * row the results will begin at.
	 *
	...
	public function orderBy($clause) {
		...
		if(isset($this->table) && 
			strpos($clause,'.') === false && strpos($name,'(') === false && 
			!array_key_exists($name, $this->selectAs)) {
	...
	protected function whereHelper() {
		...
		if(!empty($this->conditions)) {
			foreach($this->conditions as $clause) {
				...
				$columnString = $clause->column;
				if (!in_array($clause->column, array_keys($this->selectAs))) {
					$columnString = self::escapeWithTicks($columnString);
				}
				$sql .= " {$columnString} " . $clause->operator . $clause->getQueryParameter();
			}
		}
	...
	protected function joinHelper() {
		...
		if(!empty($this->joins)) {
			...
			foreach($joins as $join) {
				...
				$onStatement = ' ON ' 
					. self::escapeWithTicks($join->tablePrimaryKey) 
					. ' = ' 
					. self::escapeWithTicks($join->fromTableForeignKey);
	...
	protected static function escapeWithTicks($string) {
		...
		if(strpos($string,'.') !== false) { // Todo: Replace with Regexp
