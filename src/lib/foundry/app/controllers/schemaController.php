...
class SchemaController extends Controller {
		...
		foreach ($modelTable->getColumns() as $mc) {
				...
				if ($dbc->getName() == $mc->getName()) {
					if (($dbc->getColType() != $mc->getColType()) 
						...
						$choices = array(array(
							...
							'action'=>_furnace()->config['url_base']."_furnace/schema/editColumn/",
			...
			if (!$bFound) {
				$choices = array(array(
					...
					'action'=>_furnace()->config['url_base']."_furnace/schema/addColumn/",
					'tableName'=>$modelTable->getName(),
					'columnName'=>$mc->getName()),
					array(
					'type'=>'rename',
					'text'=>'Rename existing column: ',
					'action'=>_furnace()->config['url_base']."_furnace/schema/renameColumn/",
					'tableName'=>$modelTable->getName(),
					'renameTo'=>$mc->getName())
				);
		...
		foreach ($dbTable->getColumns() as $dbc) {	
			...
			if (!$bFound) {
				$choices[] = array(
					...
					'action'=> "_furnace/schema/deleteDbField/",
					'tableName'  =>$modelTable->getName(),
	...
	public function editColumn() {
		...
		$this->redirect("/_furnace/schema/compareFields/{$data['tableName']}");
	}
